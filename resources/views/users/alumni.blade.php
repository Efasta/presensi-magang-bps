                ...
                <div class="bg-white dark:bg-gray-800 relative sm:rounded-lg">
                    <div
                        ...
                        <div
                            ...
                            <form method="GET" action="{{ route('alumni.index') }}" id="filterForm">
                                ...
                                <div
                                    ...
                                    <div id="filterDropdown"
                                        class="absolute z-10 hidden px-3 pt-1 bg-white rounded-lg shadow w-80 dark:bg-gray-700 right-0 top-2 max-h-73 overflow-y-auto">
                                        <div
                                        ...
                                        <div id="accordion-flush" data-accordion="collapse"
                                            ...
                                            <h2 id="tahun-heading">
                                                <button type="button"
                                                    class="flex items-center justify-between w-full py-2 px-1.5 text-sm font-medium text-left text-gray-500 border-gray-200 hover:bg-gray-50 border-b"
                                                    data-accordion-target="#tahun-body" aria-expanded="false"
                                                    aria-controls="tahun-body">
                                                    <span>Tahun</span>
                                                    <svg aria-hidden="true" data-accordion-icon
                                                        class="w-5 h-5 rotate-180 shrink-0" fill="currentColor"
                                                        viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <div id="tahun-body" class="hidden" aria-labelledby="tahun-heading">
                                                <div class="py-2 font-light border-gray-300">
                                                    @php
                                                    ...
                                                    <ul class="space-y-2 mb-2">
                                                        @foreach ($years as $year)
                                                            <li class="flex items-center">
                                                                <input id="tahun-{{ $year }}" type="checkbox"
                                                                    name="tahun[]" value="{{ $year }}"
                                                                    {{ in_array($year, $selectedTahun) ? 'checked' : '' }}
                                                                    class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-emerald-600 focus:ring-emerald-500 focus:ring-2">
                                                                <label for="tahun-{{ $year }}"
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="p-4 whitespace-nowrap">Tanggal Selesai</th>
                                <th scope="col" class="p-4 items-center whitespace-nowrap">Aksi</th>
                            </tr>
                        </thead>
                        <tbody id="userTableBody">
                            @forelse ($users as $user)
                                <tr class="border-b border-gray-200 dark:border-gray-600 dark:hover:bg-gray-700"
                                    ...
                                    <th scope="row"
                                        class="user-name px-4 py-3 font-medium text-black whitespace-nowrap dark:text-white">
                                    ...
                                    <td class="px-4 py-3">
                                        <span
                                            class="bg-green-100 text-green-800 text-xs font-semibold px-2 py-0.5 rounded">
                                            Selesai
                                        </span>
                                    </td>

                                    <!-- Kolom tanggal_keluar -->
                                    <td class="px-4 py-3">
                                        {{ $user->tanggal_keluar ? \Carbon\Carbon::parse($user->tanggal_keluar)->format('d M Y') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 flex items-center">
                                        ...
                                        <div id="users-{{ $user->id }}-dropdown"
                                            ...
                                            <ul class="py-1 text-sm"
                                                ...
                                                <li>
                                                    <a href="{{ route('alumni.show', ['user' => $user->slug, 'from' => 'alumni']) }}"
                                                ...
                                                <li>
                                                    <a href="{{ route('alumni.showDetail', ['user' => $user->slug, 'from' => 'alumni']) }}"
                                                        class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                ...
                                                <li>
                                                    <a href="{{ route('alumni.edit', ['user' => $user->slug, 'from' => 'alumni']) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-50 text-gray-500 italic">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function resetFilter() {
            ...
            document.querySelectorAll('#jenis_kelamin-body input[type="checkbox"]').forEach(cb => cb.checked =
                false);
            document.querySelectorAll('#kehadiran-body input[type="checkbox"]').forEach(cb => cb.checked =
                false);
        ...
        function applyFilter() {
            const checkedFunctions = Array.from(document.querySelectorAll(
                '#fungsi-body input[type="checkbox"]:checked')).map(cb => cb.value.toLowerCase());

            const checkedStatuses = Array.from(document.querySelectorAll(
                '#kehadiran-body input[type="checkbox"]:checked')).map(cb => cb.value.toLowerCase());
            ...
            rows.forEach(row => {
                ...
                row.style.display = (fungsiMatch && statusMatch && nameMatch) ? '' : 'none';
            });
        ...
        // Klik Reset filter
        const resetLink = document.querySelector('#filterDropdown a[href="/absensi"]');
        if (resetLink) {
        ...
        if (dropdownButton && dropdown) {
            ...
            document.addEventListener('click', function(event) {
                // biar sidebar tetap bisa diklik
                if (event.target.closest('aside')) return; // jangan tutup dropdown saat klik sidebar
